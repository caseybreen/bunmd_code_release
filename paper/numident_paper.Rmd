...
|        The NARA Numident is an individual-level mortality data set with over 49 million death records. The microdata includes variables describing race, sex, birthplace, and ZIP Code at time of death. It also contains administrative variables, such as age of first Social Security application and total number of Social Security applications. Notably, the death coverage is nearly complete for 1988-2005. The records can be linked to other data sets, either by Social Security number or a combination of other key identifiers. However, mortality research with the Numident records presents challenges. Survivorship cannot be estimated, as the deaths do not have denominators. Further, the observed deaths are left and right truncated — this makes calculating unbiased estimates of mortality difficult. We introduce methods to overcome this. Finally, the Numident files lack any indicator of socioeconomic status; without linking to another data source, there is no way to obtain measures of education or income.  
...
|        Another publicly available data resource for mortality research is the Social Security Death Master File (DMF). The DMF was first made available in 1988, and is extracted quarterly from the Numident records [@hill_social_nodate]. The file has been used by some researchers to study mortality, particularly at older ages [@gavrilov_mortality_2012]. While the file has high death coverage for deaths of persons aged 65 or older from 1975 - 2005, it lacks covariates, such as sex, race, or place of birth [@hill_social_nodate]. The methods we provide here are also useful for researchers working with the DMF. 
...
|        The NARA Numident records contain three types of entries: applications, claims, and deaths. The Social Security Administration adds a new entry to the Numident when a Social Security cardholder submits a new application or claim. New entries never overlay old entries. Instead, a new entry is added to the Numident. Figure 1 shows the distribution of application and claim entries per SSN. While 43.3% of individuals in the Numident have multiple application entries, only 0.3% of individuals have multiple claims. 

\begin{figure}
  \centering
  \subfloat[\label{fig:tp01}]{\includegraphics[width=7in]{/Users/casey/Documents/censoc/numident_paper/illustrations/entries_per_ssn.png}}
  \caption{Number of entries per SSN for the Numident Application and Claims files.}
  \label{fig:tp}
\end{figure}
...
\begin{figure}
  \centering
  \subfloat[\label{fig:tp01}]{\includegraphics[width=5in]{../illustrations/death_coverage_bunmd.png}}
  \caption{Berkeley Unified Numident Mortality Database creation flowchart. }
  \label{fig:tp}
\end{figure}
...
\begin{figure}
  \centering
  \subfloat[\label{fig:tp01}]{\includegraphics[width=7in]{../illustrations/lexis_combine_plot.png}}
  \caption{Berkeley Unified Numident Mortality Database creation flowchart. }
  \label{fig:tp}
\end{figure}
...
\begin{figure}
  ...
  \caption{ZIP Codes surrounding Cleveland.}
  \label{fig:tp}
\end{figure}
